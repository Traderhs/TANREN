(function(global, factory) {
	...
	*/
	function createFep(cb, opts) {
		...
		let kana = "";
		let pend = "";
		// TANREN: keep a real caret inside the unconverted yomi, matching desktop IME editing.
		let caret = 0;
		let segs = null;
		let focus = 0;
		let genId = 0;
		...
		const clampCaret = () => {
			caret = Math.max(0, Math.min(caret, kanaChars().length));
		};
		const splitKanaAtCaret = () => {
			clampCaret();
			const chars = kanaChars();
			return [chars.slice(0, caret).join(""), chars.slice(caret).join("")];
		};
		const displayYomi = () => {
			const [left, right] = splitKanaAtCaret();
			return left + pend + right;
		};
		const displayCaret = () => {
			const [left] = splitKanaAtCaret();
			return Array.from(left + pend).length;
		};
		const flushPendAtCaret = (flush = true) => {
			if (!pend) return;
			const [left, right] = splitKanaAtCaret();
			const r = resolveRomaji(left, pend, flush);
			kana = r.kana + right;
			caret = Array.from(r.kana).length;
			pend = r.pend;
		};
		const insertTextAtCaret = (text) => {
			const [left, right] = splitKanaAtCaret();
			kana = left + text + right;
			caret = Array.from(left + text).length;
		};
		const resetAddl = () => {
			addlShown = 0;
			addlSel = null;
		...
		const suggestYomi = () => {
			if (!composing()) return "";
			const [left, right] = splitKanaAtCaret();
			return resolveRomaji(left, pend, true).kana + right;
		};
		...
		*/
		function builtinNav(e) {
			...
			if (k === "ArrowLeft" || k === "ArrowRight") {
				...
				if (segs && segs.length > 1) {
					...
					flushPendAtCaret(true);
					caret = Math.max(0, Math.min(kanaChars().length, caret + (k === "ArrowRight" ? 1 : -1)));
					render();
				}
				return true;
			}
			if (k === "Home" || k === "End") {
				if (!composing() || segs) return false;
				flushPendAtCaret(true);
				caret = k === "Home" ? 0 : kanaChars().length;
				render();
				return true;
			}
			if (k === "Delete") {
		...
		function feed(e) {
			...
			if (k.length === 1 && k >= " " && k <= "~") {
				...
				pend += ch;
				const [left, right] = splitKanaAtCaret();
				const r = resolveRomaji(left, pend, false);
				kana = r.kana + right;
				caret = Array.from(r.kana).length;
				pend = r.pend;
				genId++;
		...
		function insertKana(text, replaceCount = 0) {
			...
			if (replaceCount > 0) {
				const chars = kanaChars();
				if (caret < replaceCount) return false;
				chars.splice(caret - replaceCount, replaceCount);
				kana = chars.join("");
				caret -= replaceCount;
			}
