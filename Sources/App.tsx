...
function DeckList({ decks, onRefresh, onEdit, onOpenedDeckChange, onRequestHomeSection, audioSettings }: {
  ...
  const filteredBookEntries = entrySearch.trim()
    ...
    : bookEntries;
  const parsedBulkEntries = useMemo(() => parseEntryText(bulkText), [bulkText]);
  ...
  useEffect(() => {
    ...
    void api.stageSchedules(openedDeck.id, stages).catch(async () => {
      // Keep valid stage summaries available when an individual snapshot is corrupt.
      const schedules = await Promise.all(stages.map((stage) => api.stageSchedule(openedDeck.id, stage).catch(() => null)));
      return schedules.filter((schedule): schedule is StageScheduleSummary => schedule !== null);
    }).then((schedules) => {
      if (disposed) return;
      const next: Record<number, StageScheduleSummary> = {};
      for (const schedule of schedules) next[schedule.stage] = schedule;
      setStageSchedules(next);
    }).catch(() => { if (!disposed) setStageSchedules({}); });
